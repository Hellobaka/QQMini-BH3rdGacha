...
{
    public class JZB10 : IOrderModel
    {
        public string GetOrderStr()
        {
            return PublicArgs.order_JZB10;
        }
        public bool Judge(string destStr)
        {
            return destStr.Replace(" ", "").ToUpper() == GetOrderStr()
                && Save.AppConfig.Object["ExtraConfig"]["SwitchJZB10"].GetValueOrDefault("1") != "0";
        }
        public FunctionResult Progress(QMGroupMessageEventArgs e)
        {
            ...
            QMApi.CurrentApi.SendGroupMessage(e.RobotQQ, e.FromGroup, PublicArgs.JZB10
                .Replace("<@>", $"[@{e.FromQQ.Id}]").Replace("<#>", diamond.ToString()));
            List<GachaResult> ls = new List<GachaResult>();

            for (int i = 0; i < 10; i++)
            {
                ls.Add(MainGacha.JZ_GachaMain(PublicArgs.PoolName.精准B));
            ...
            sendText.MsgToSend.Add(ini.Object["详情"]["B_ResultAt"].GetValueOrDefault("0") == "1"
                ...
                ? Helper.TextGacha(ls) : $"[pic=" +
                    $"{cp.MakePic(ls, PublicArgs.PoolName.精准B, diamond - 2800, e.FromQQ.Id, e.FromGroup.Id)}]");
            return result;
        }
