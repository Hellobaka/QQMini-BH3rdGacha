...
namespace BH3rdGacha.Rank
{
    ...
    public class WeekRank
    {
        ...
        /// <param name="e"></param>
        /// <returns></returns>
        public static StringBuilder GetWeekRankText(SQLiteConnection cn, QMGroupMessageEventArgs e)
        {
            ...
            sb.AppendLine($"抽卡次数最多的是:{sr.GetInt64(1)} 一共抽了{sr.GetInt32(0)}次 共花费了{sr.GetInt32(0) * 280}水晶");
            ...
            sb.AppendLine($"最欧的是:[@{ls[0].qqid}] 综合出货率为{(ls[0].purple_count / (double)ls[0].gacha_count * 100):0.000}%");
