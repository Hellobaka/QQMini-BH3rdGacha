...
{
    public class JZA10 : IOrderModel
    {
        public string GetOrderStr()
        {
            return PublicArgs.order_JZA10;
        }
        public bool Judge(string destStr)
        {
            return destStr.Replace(" ", "").ToUpper() == GetOrderStr()
                && Save.AppConfig.Object["ExtraConfig"]["SwitchJZA10"].GetValueOrDefault("1") != "0";
        }
        public FunctionResult Progress(QMGroupMessageEventArgs e)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ls.Add(MainGacha.JZ_GachaMain(PublicArgs.PoolName.精准A));
            ...
            ini.Load();
            sendText.MsgToSend.Add(ini.Object["详情"]["A_ResultAt"].GetValueOrDefault("0") == "1"
                ? $"[@{e.FromQQ.Id}]" : ""
                + Save.AppConfig.Object["ExtraConfig"]["TextGacha"].GetValueOrDefault("0") == "1"
                ? Helper.TextGacha(ls) : $"[pic=" +
                    $"{cp.MakePic(ls, PublicArgs.PoolName.精准A, diamond - 2800, e.FromQQ.Id, e.FromGroup.Id)}]");
            return result;
        }
